...
export default function Page({
	...
	return (
		<>
			<NavBar logo={empresa.logo} />
			<div className="flex justify-center">
				<div className="max-w-[700px] px-4 sm:px-6 lg:px-8 mt-20 pb-20">
					{landingData.estructura
						// @ts-ignore
						.filter((item) => item.p === product.id)
						// @ts-ignore
						.map((item, index) => {
							switch (parseInt(item.c)) {
								...
								case 2:
									return (
										product.video && (
											<section key={index} className="video">
								...
								case 4:
									return (
										<section key={index} className="galeria">
											<div className="w-full sm:w-[480px] md:w-[580px] h-[400px] relative mt-6 mx-auto mb-8">
												{showCarousel ? (
													<Carousel>
														{imageUrls.map((url, imgIndex) => (
															<div key={imgIndex} className="h-[400px]">
